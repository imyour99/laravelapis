...
<div class="rounded-corner bg-white col-white1" id="original_intervention">
    <hr class="tour-gray-hr" />

    @php
        $interventions_total = [];
        $interventions_success_array = [];
        $interventions_semi_ko_array = [];
        $interventions_ko_array = [];
    @endphp
    ...
    @foreach ($collaborations as $collaboration)
        <div class="tour-row">
            ...
            @php
                ...
                if (isset($_GET['date'])) {
                    ...
                    $interventions_count = DB::table('interventions')
                        ->select('interventions.user_id', 'interventions.client_id', 'interventions.assignee_id', 'interventions.schedule', 'interventions.status as statusv', DB::raw('llx_client.id as client_id'), 'llx_client.intervention_status', 'llx_client.pdl', 'llx_client.prenom', 'llx_client.nom', 'llx_client.adress', 'llx_client.status', 'llx_client.intervention_status', 'llx_client.visite_status', 'llx_client.image')
                    ...
                    $interventions_success = DB::table('interventions')
                        ->select('interventions.user_id', 'interventions.client_id', 'interventions.assignee_id', 'interventions.schedule', 'interventions.status as statusv', DB::raw('llx_client.id as client_id'), 'llx_client.intervention_status', 'llx_client.pdl', 'llx_client.prenom', 'llx_client.nom', 'llx_client.adress', 'llx_client.status', 'llx_client.intervention_status', 'llx_client.visite_status', 'llx_client.image')
                        ->leftJoin('llx_client', 'llx_client.id', 'interventions.client_id')
                        ->whereRaw('assignee_id = ' . $collaboration->id . " and date_format(schedule,'%Y-%m-%d') = '" . $selected_date . "' and schedule > '" . $selected_date . " 06:00:00'")
                        ->orderBy('schedule', 'asc')
                        ->where('interventions.status', '4')
                        ->count();
                    $interventions_semi_ko = DB::table('interventions')
                        ->select('interventions.user_id', 'interventions.client_id', 'interventions.assignee_id', 'interventions.schedule', 'interventions.status as statusv', DB::raw('llx_client.id as client_id'), 'llx_client.intervention_status', 'llx_client.pdl', 'llx_client.prenom', 'llx_client.nom', 'llx_client.adress', 'llx_client.status', 'llx_client.intervention_status', 'llx_client.visite_status', 'llx_client.image')
                        ->leftJoin('llx_client', 'llx_client.id', 'interventions.client_id')
                        ->whereRaw('assignee_id = ' . $collaboration->id . " and date_format(schedule,'%Y-%m-%d') = '" . $selected_date . "' and schedule > '" . $selected_date . " 06:00:00'")
                        ->orderBy('schedule', 'asc')
                        ->where('interventions.status', '3')
                        ->count();
                    $interventions_ko = DB::table('interventions')
                        ->select('interventions.user_id', 'interventions.client_id', 'interventions.assignee_id', 'interventions.schedule', 'interventions.status as statusv', DB::raw('llx_client.id as client_id'), 'llx_client.intervention_status', 'llx_client.pdl', 'llx_client.prenom', 'llx_client.nom', 'llx_client.adress', 'llx_client.status', 'llx_client.intervention_status', 'llx_client.visite_status', 'llx_client.image')
                        ->leftJoin('llx_client', 'llx_client.id', 'interventions.client_id')
                        ->whereRaw('assignee_id = ' . $collaboration->id . " and date_format(schedule,'%Y-%m-%d') = '" . $selected_date . "' and schedule > '" . $selected_date . " 06:00:00'")
                        ->orderBy('schedule', 'asc')
                    ...
                    <div class="tour-right-row-div">
                        ...
                        <div class="tour-right-row  blue-border">
                            ...
                            @foreach ($interventions as  $intervention)
                                @php
                                    $hour = date('H', strtotime($intervention->schedule));
                                    $endhour = $hour + 2;

                                    $currentTime = date('Y-m-d H');
                                    $schedule = date('Y-m-d H', strtotime($intervention->schedule));
                                @endphp
                                ...
                                @if ($schedule <= $currentTime)
                                    ...
                                    <div class="tour-box">
                                        {{-- <div class="tour-box-blue tour-box-half tour-col-9 " style="{{$val}}"> --}}
                                        <div class="tour-box-blue tour-box-half tour-col-9">
                                            <strong class="tour-left-row-text text-black">{{ $intervention->prenom }}
                                                {{ $intervention->nom }}</strong>
                                        ...
                                        <div class="notificationPopUp">
                                            <div class=" card-body p-0" style="">
                                                <div class="w-75"
                                                    ...
                                                    <h6>{{ $intervention->prenom }} {{ $intervention->nom }}</h6>
        ...
        @php
            ...
            array_push($interventions_total, $interventions_count);
            array_push($interventions_success_array, $interventions_success);
            array_push($interventions_semi_ko_array, $interventions_semi_ko);
            array_push($interventions_ko_array, $interventions_ko);
    ...
    {{-- <div class="tour-num-top-div tour-num-top3 d-flex" style="position: absolute;bottom: 0;">
                    <span class="tour-num-top text-blue" style="position: absolute;bottom: 15px; left: -5px;height: 30px;width: 30px;padding: 5px;">300</span>
...
<div class="tour-num-top-div tour-num-top3 d-flex" style="position: absolute;bottom: 0;">
    <span class="tour-num-top text-blue"
        style="position: absolute;bottom: 15px; left: -5px;height: 30px;width: 30px;padding: 5px;">{{ array_sum($interventions_total) }}
    </span>
    <span class="ml-3 text-blue font-weight-bold tour-label"
        style="position: absolute;bottom: 20px;left: 20px;">Prévisionel</span>
</div>
...
<div class="text-right float-right" style="position:absolute;bottom:20px;right:20px">
    <span class="bottom-small-text green-text">Succés </span><span
        class="bottom-circle bottom-green-circle-count">{{ array_sum($interventions_success_array) }}</span>
    <span class="bottom-small-text orange-text">Semi KO </span><span
        class="bottom-circle bottom-orange-circle-count">{{ array_sum($interventions_semi_ko_array) }}</span>
    <span class="bottom-small-text red-text">KO </span><span
        class="bottom-circle bottom-green-red-count">{{ array_sum($interventions_ko_array) }}</span>
</div>
